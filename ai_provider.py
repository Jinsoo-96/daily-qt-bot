from google import genai  # 라이브러리 이름이 'google-genai'이지만 import는 'google.genai' 혹은 'google'에서 합니다.
from google.genai import types
import os
import time

def get_ai_reflection(bible_title, bible_range, content_body):
    client = genai.Client(api_key=os.getenv("GEMINI_API_KEY"))
    
    prompt = f"""
당신은 두 역할을 동시에 수행합니다.

1) 성경 중심의 기독교 세계관 위에서,
   복음주의 신학 전통에 기반한 뛰어난 성서신학자
2) 따뜻하고 문학적인 묵상 에세이를 쓰는 작가

아래에 제공된 [성경 본문]을 기반으로
오늘 하루를 위한 큐티 묵상 글을 작성한다.

[작성 규칙]
- 전체 분량은 공백 포함 1900자 이내로 작성한다. 
- 한국어로 작성한다
- 과도한 설교체나 교훈적 표현은 피하고, 차분하고 따뜻한 톤을 유지한다
- 신학적 해설과 묵상 에세이가 자연스럽게 하나의 흐름으로 이어지도록 한다
- 본문에 없는 내용을 단정적으로 추가하지 않는다
- 독자의 일상과 연결되는 묵상적 적용을 포함한다
- 20~30대 신앙인이 겪는 현실적인 고민(진로, 관계, 불안, 반복되는 일상 등)을 은근히 반영한다
- 20대 초반부터 30대 후반까지 모두 공감할 수 있는, 특정 삶의 단계에 치우치지 않은 중립적인 일상 언어를 사용한다
- 독자를 다그치거나 평가하지 않는다
- 디스코드에서 사용할 수 있는 마크다운 형식으로 가독성 좋게 작성한다.


[출력 형식 — 반드시 그대로 따른다 아래 나열된 것 외에 그 어떤 카테고리 생성 금지]

## 오늘을 위한 묵상: (본문을 반영한 짧고 따뜻한 제목)

### 본문 해설: 소제목
> 하나의 인용 블록 안에서 줄바꿈을 사용해 문단을 구분한다(인용 블록이 끊어지지 않도록 주의할 것)
> 성경 중심의 기독교 세계관 위에서, 뛰어난 신학자의 분석으로 논리적으로 작성한다
> 내용

### 묵상 에세이: 하위 내용에 어울리는 소제목을 반드시 생성한다
> 하나의 인용 블록 안에서 줄바꿈을 사용해 문단을 구분한다(인용 블록이 끊어지지 않도록 주의할 것)
> 따뜻하고 문학적인 묵상 에세이를 쓰는 작가의 느낌으로 작성한다
> 내용

### 하루를 위한 묵상 질문 3가지
1. **소제목**
   > 질문 내용

2. **소제목**
   > 질문 내용

3. **소제목**
   > 질문 내용


[작성 내용 상세]

### 본문 해설: 하위 내용에 어울리는 소제목을 반드시 생성한다
> 본문의 역사적·문맥적 의미와 핵심 신학 주제를 담는다
> 본문 해설은 최소 2~3개의 문단으로 충분히 풀어낸다
> 본문 해설에서 묵상 에세이로 넘어갈 때, 의미의 다리 역할을 하는 전환 문단을 포함한다

### 묵상 에세이: 하위 내용에 어울리는 소제목을 반드시 생성한다
> 오늘 하루를 살아가는 신앙인의 마음에 조용히 말을 건네는 문체로 작성한다
> 묵상 에세이는 서두–중심–여운의 흐름으로 비교적 충분히 서술한다
> 묵상 에세이의 마지막에는, 필요할 경우 여운을 남기는 짧은 단독 문단을 둘 수 있다

### 하루를 위한 묵상 질문 3가지
> 각 질문은 생각을 열어두는 질문으로 작성한다
> 하루를 정리하거나 결론짓는 느낌은 피한다
> 각 질문은 답을 요구하지 않고, 생각과 기도가 이어지도록 여백을 남긴다
> 생각해볼 만한 고민이나, 하루를 시작하거나 품고 갈 수 있는 질문으로 구성한다
> 결단·마무리·평가형 질문 금지


[성경 본문]
{bible_range}
{content_body}
"""
    max_retries = 3  # 최대 3번 시도
    retry_delay = 2  # 실패 시 2초 대기 후 다시 시도
    # 안전 설정: 성경 본문 분석 중 차단되는 일을 방지합니다.
    # 2. 안전 설정 (확실한 최신 문법입니다)
    # 성경 본문의 특정 단어가 차단되는 것을 방지하기 위해 BLOCK_NONE으로 설정합니다.
    config = types.GenerateContentConfig(
        safety_settings=[
            types.SafetySetting(category='HARM_CATEGORY_HARASSMENT', threshold='BLOCK_NONE'),
            types.SafetySetting(category='HARM_CATEGORY_HATE_SPEECH', threshold='BLOCK_NONE'),
            types.SafetySetting(category='HARM_CATEGORY_DANGEROUS_CONTENT', threshold='BLOCK_NONE'),
            types.SafetySetting(category='HARM_CATEGORY_SEXUALLY_EXPLICIT', threshold='BLOCK_NONE'),
        ]
    )
    
    for attempt in range(max_retries):
        try:
            print(f"🤖 AI 응답 생성 중... (시도 {attempt + 1}/{max_retries})")
            # 3. 콘텐츠 생성 (최신 문법: client.models.generate_content)
            response = client.models.generate_content(
                model='gemini-3-flash-preview',
                contents=prompt,
                config=config)
            
            # 응답이 성공적으로 왔고 텍스트가 있는 경우
            if response and response.text:
               # 1. 일단 모든 '>' 뒤에 공백을 붙입니다.
               content = response.text.replace('>', '> ')
               
               # 2. 혹시 이미 공백이 있어서 '>  '가 되어버린 경우를 대비해 다시 '> '로 정렬합니다.
               fixed_content = content.replace('>  ', '> ')
               
               return fixed_content
            
            # 응답은 왔으나 텍스트가 없는 경우 (Safety Filter 작동 등)
            else:
                print("⚠️ AI 응답이 비어있습니다. 다시 시도합니다.")
                
        except Exception as e:
            print(f"❌ 시도 {attempt + 1} 실패: {e}")
            if attempt < max_retries - 1:
                time.sleep(retry_delay) # 잠시 대기 후 다음 루프 진행
            else:
                # 모든 시도가 실패했을 때 보낼 메시지
                return "🙏 현재 AI가 묵상을 준비하는 데 어려움을 겪고 있습니다. 잠시 후 다시 확인해 주세요."

    return "🙏 묵상 내용을 생성할 수 없습니다."
